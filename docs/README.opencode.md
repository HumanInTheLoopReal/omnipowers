...
```json
{
  "plugin": ["omnipowers@git+https://github.com/HumanInTheLoopReal/omnipowers.git"]
}
```
...
another harness, install Omnipowers separately for each one.
...
OpenCode installs Omnipowers through a git-backed package spec. Some OpenCode
and Bun versions pin that resolved git dependency in a lockfile or cache, so a
restart may not pick up the newest Omnipowers commit. If updates do not appear,
...
```json
{
  "plugin": ["omnipowers@git+https://github.com/HumanInTheLoopReal/omnipowers.git#v5.0.3"]
}
```
...
1. **Injects bootstrap context** via the `experimental.chat.messages.transform` hook, adding omnipowers awareness to every conversation.
...
```powershell
npm install omnipowers@git+https://github.com/HumanInTheLoopReal/omnipowers.git --prefix "$HOME\.config\opencode"
```
...
```json
{
  "plugin": ["~/.config/opencode/node_modules/omnipowers"]
}
```
...
- Report issues: https://github.com/HumanInTheLoopReal/omnipowers/issues
- Main documentation: https://github.com/HumanInTheLoopReal/omnipowers
