...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$OMNIPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$OMNIPOWERS_SKILLS_DIR/using-omnipowers/SKILL.md" "$TEST_HOME/using-omnipowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$OMNIPOWERS_PLUGIN_FILE" missing
}
