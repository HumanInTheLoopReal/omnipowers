...
const extensionPath = resolve(repoRoot, '.pi/extensions/omnipowers.ts');
const piToolsPath = resolve(repoRoot, 'skills/using-omnipowers/references/pi-tools.md');
...
test('package.json declares a pi package with skills and extension resources', async () => {
  ...
  assert.equal(pkg.name, 'omnipowers');
  assert.ok(pkg.keywords.includes('pi-package'));
  assert.deepEqual(pkg.pi.skills, ['./skills']);
  assert.deepEqual(pkg.pi.extensions, ['./.pi/extensions/omnipowers.ts']);
});
...
test('startup context injects the bootstrap as one user message until agent_end', async () => {
  ...
  assert.match(textOf(result.messages[0]), /You have omnipowers/);
  ...
  assert.match(textOf(repeatedProviderRequest.messages[0]), /You have omnipowers/);
...
test('session_compact injects bootstrap after compaction summaries, not before compaction', async () => {
  ...
  assert.match(textOf(result.messages[1]), /You have omnipowers/);
