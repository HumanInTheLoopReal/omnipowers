...
- **Legacy slash commands removed** — `/brainstorm`, `/execute-plan`, and `/write-plan` are gone. They were deprecated stubs that did nothing but tell the user to invoke the corresponding skill. Invoke `omnipowers:brainstorming`, `omnipowers:executing-plans`, and `omnipowers:writing-plans` directly instead. (#1188)
- **`omnipowers:code-reviewer` named agent removed** — the agent was the plugin's only named agent and was used by exactly two skills, while every other reviewer/implementer subagent in the repo dispatches `general-purpose` with a prompt template alongside its skill. The agent's persona and checklist have been merged into `skills/requesting-code-review/code-reviewer.md` as a self-contained Task-dispatch template. Anyone dispatching `Task (omnipowers:code-reviewer)` should switch to `Task (general-purpose)` with the prompt template instead. (PR #1299)
...
- **Provenance-based cleanup** — `finishing-a-development-branch` only cleans up worktrees inside `.worktrees/` (created by omnipowers); anything outside is left alone. Fixes #940 (Option 2 was incorrectly cleaning up worktrees), #999 (merge-then-remove ordering), and #238 (`cd` to repo root before `git worktree remove`).
...
- **New harness PRs require a session transcript** — most past new-harness integrations copied skill files or wrapped with `npx skills` instead of loading the `using-omnipowers` bootstrap at session start. The acceptance test ("Let's make a react todo list" must auto-trigger `brainstorming` in a clean session) and a complete transcript are now required.

### Codex Plugin Mirror Tooling

New `sync-to-codex-plugin` script mirrors omnipowers into the OpenAI Codex plugin marketplace as `prime-radiant-inc/openai-codex-plugins`. Path/user-agnostic so any team member can run it. (PR #1165)
...
- **SessionStart context injection** — Copilot CLI v1.0.11 added support for `additionalContext` in sessionStart hook output. The session-start hook now detects the `COPILOT_CLI` environment variable and emits the SDK-standard `{ "additionalContext": "..." }` format, giving Copilot CLI users the full omnipowers bootstrap at session start. (Original fix by @culinablaz in PR #910)
- **Tool mapping** — added `references/copilot-tools.md` with the full Claude Code to Copilot CLI tool equivalence table
- **Skill and README updates** — added Copilot CLI to the `using-omnipowers` skill's platform instructions and README installation section

### OpenCode Fixes

- **Skills path consistency** — the bootstrap text no longer advertises a misleading `configDir/skills/omnipowers/` path that didn't match the runtime path. The agent should use the native `skill` tool, not navigate to files by path. Tests now use consistent paths derived from a single source of truth. (#847, #916)
...
This fix should dramatically improve omnipowers skills compliance and should reduce the chances of Claude entering its native plan mode unintentionally.
...
**Using-omnipowers workflow graph intercepts EnterPlanMode**
...
Changed `async: true` to `async: false` in hooks.json. When async, the hook could fail to complete before the model's first turn, meaning using-omnipowers instructions weren't in context for the first message.
...
The `omnipowers-codex` bootstrap CLI, Windows `.cmd` wrapper, and related bootstrap content file have been removed. Codex now uses native skill discovery via `~/.agents/skills/omnipowers/` symlink, so the old `use_skill`/`find_skills` CLI tools are no longer needed.
...
The synchronous SessionStart hook blocked the TUI from entering raw mode on Windows, freezing all keyboard input. Running the hook async prevents the freeze while still injecting omnipowers context.
...
- Windows doesn't respect shebangs, so directly invoking the extensionless `omnipowers-codex` script triggered an "Open with" dialog. All invocations now prefixed with `node`.
...
Omnipowers for OpenCode now uses OpenCode's native `skill` tool instead of custom `use_skill`/`find_skills` tools. This is a cleaner integration that works with OpenCode's built-in skill discovery.

**Migration required:** Skills must be symlinked to `~/.config/opencode/skills/omnipowers/` (see updated installation docs).
...
**Strengthened using-omnipowers skill for explicit skill requests**
...
The underlying skills (`omnipowers:brainstorming`, `omnipowers:executing-plans`, `omnipowers:writing-plans`) remain available for Claude to invoke autonomously. This change prevents confusion when Claude would invoke a command that just redirects to a skill anyway.
...
Fixed a confusing pattern where Claude would invoke a skill via the Skill tool, then try to Read the skill file separately. The `using-omnipowers` skill now explicitly states that the Skill tool loads skill content directly—no need to read files.

- Added "How to Access Skills" section to `using-omnipowers`
- Changed "read the skill" → "invoke the skill" in instructions
- Updated slash commands to use fully qualified skill names (e.g., `omnipowers:brainstorming`)
...
**Skill priority in using-omnipowers**
...
- **Rationalizations table** in using-omnipowers - Scannable format including new entries: "I need more context first", "Let me explore first", "This feels productive"
...
- **OpenCode Bootstrap Refactor**: Switched from `chat.message` hook to `session.created` event for bootstrap injection
  ...
  - Explicitly tells the model that using-omnipowers is already loaded to prevent redundant skill loading
...
- **OpenCode Support**: Native JavaScript plugin for OpenCode.ai
  ...
  - Three-tier skill priority: project > personal > omnipowers
...
- Optimized omnipowers bootstrap to eliminate redundant skill execution. The `using-omnipowers` skill content is now provided directly in session context, with clear guidance to use the Skill tool only for other skills. This reduces overhead and prevents the confusing loop where agents would execute `using-omnipowers` manually despite already having the content from session start.
...
- Added unified `omnipowers-codex` script with bootstrap/use-skill/find-skills commands
- Cross-platform Node.js implementation (works on Windows, macOS, Linux)
- Namespaced skills: `omnipowers:skill-name` for omnipowers skills, `skill-name` for personal
...
- Updated terminology: "Omnipowers skills" instead of "Core skills"

### Files Added
- `.codex/INSTALL.md` - Installation guide for Codex users
- `.codex/omnipowers-bootstrap.md` - Bootstrap instructions with Codex adaptations
- `.codex/omnipowers-codex` - Unified Node.js executable with all functionality

**Note:** Codex support is experimental. The integration provides core omnipowers functionality but may require refinement based on user feedback.

## v3.2.3 (2025-10-23)

### Improvements

**Updated using-omnipowers skill to use Skill tool instead of Read tool**
...
### Files Changed
- Updated: `skills/using-omnipowers/SKILL.md` - Changed tool references from Read to Skill

## v3.2.2 (2025-10-21)

### Improvements

**Strengthened using-omnipowers skill against agent rationalization**
...
### Files Changed
- Updated: `skills/using-omnipowers/SKILL.md` - Added three layers of enforcement to prevent skill-skipping rationalization
...
- Added `omnipowers:code-reviewer` agent to plugin's `agents/` directory
- Agent provides systematic code review against plans and coding standards
- Previously required users to have personal agent configuration
- All skill references updated to use namespaced `omnipowers:code-reviewer`
- Fixes #55

### Files Changed
- New: `agents/code-reviewer.md` - Agent definition with review checklist and output format
- Updated: `skills/requesting-code-review/SKILL.md` - References to `omnipowers:code-reviewer`
- Updated: `skills/subagent-driven-development/SKILL.md` - References to `omnipowers:code-reviewer`
...
- All internal skill references now use `omnipowers:` namespace prefix
- Updated format: `omnipowers:test-driven-development` (previously just `test-driven-development`)
...
- **Fixed command syntax in README** (#44) - Updated all command references to use correct namespaced syntax (`/omnipowers:brainstorm` instead of `/brainstorm`). Plugin-provided commands are automatically namespaced by Claude Code to avoid conflicts between plugins.
...
# Omnipowers v2.0.0 Release Notes

## Overview

Omnipowers v2.0 makes skills more accessible, maintainable, and community-driven through a major architectural shift.

The headline change is **skills repository separation**: all skills, scripts, and documentation have moved from the plugin into a dedicated repository ([HumanInTheLoopReal/omnipowers-skills](https://github.com/HumanInTheLoopReal/omnipowers-skills)). This transforms omnipowers from a monolithic plugin into a lightweight shim that manages a local clone of the skills repository. Skills auto-update on session start. Users fork and contribute improvements via standard git workflows. The skills library versions independently from the plugin.
...
**The biggest change:** Skills no longer live in the plugin. They've been moved to a separate repository at [HumanInTheLoopReal/omnipowers-skills](https://github.com/HumanInTheLoopReal/omnipowers-skills).

**What this means for you:**

- **First install:** Plugin automatically clones skills to `~/.config/omnipowers/skills/`
...
1. Your old `~/.config/omnipowers/.git` will be backed up to `~/.config/omnipowers/.git.bak`
2. Old skills will be backed up to `~/.config/omnipowers/skills.bak`
3. Fresh clone of HumanInTheLoopReal/omnipowers-skills will be created at `~/.config/omnipowers/skills/`

### Removed Features

- **Personal omnipowers overlay system** - Replaced with git branch workflow
- **setup-personal-omnipowers hook** - Replaced by initialize-skills.sh
...
- Clones HumanInTheLoopReal/omnipowers-skills on first run
...
- `OMNIPOWERS_SKILLS_ROOT` set to `~/.config/omnipowers/skills`
...
- Removed obsolete setup-personal-omnipowers call from session-start
...
- Prominent link to omnipowers-skills repo
...
**Removed:**
- `skills/` directory (82 files) - Now in HumanInTheLoopReal/omnipowers-skills
- `scripts/` directory - Now in HumanInTheLoopReal/omnipowers-skills/skills/using-skills/
- `hooks/setup-personal-omnipowers.sh` - Obsolete

**Modified:**
- `hooks/session-start.sh` - Use skills from ~/.config/omnipowers/skills
- `commands/brainstorm.md` - Updated paths to OMNIPOWERS_SKILLS_ROOT
- `commands/write-plan.md` - Updated paths to OMNIPOWERS_SKILLS_ROOT
- `commands/execute-plan.md` - Updated paths to OMNIPOWERS_SKILLS_ROOT
...
- PR #2: Personal omnipowers overlay system (later replaced)
...
1. **Backup your personal skills** (if you have any):
   ```bash
   cp -r ~/.config/omnipowers/skills ~/omnipowers-skills-backup
   ```
...
- Skills repository is now at https://github.com/HumanInTheLoopReal/omnipowers-skills
...
**Full Changelog:** https://github.com/HumanInTheLoopReal/omnipowers/compare/dd013f6...main
**Skills Repository:** https://github.com/HumanInTheLoopReal/omnipowers-skills
