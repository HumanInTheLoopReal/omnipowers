...
mkdir -p "$OMNIPOWERS_SKILLS_DIR/priority-test"
cat > "$OMNIPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$OMNIPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
...
# Test 2: Document personal vs bundled omnipowers priority
echo ""
echo "Test 2: Documenting personal vs omnipowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled omnipowers version instead of personal version"

# Test 3: Document project vs bundled omnipowers priority
echo ""
echo "Test 3: Documenting project vs personal/omnipowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled omnipowers version instead of project version"

# Test 4: Test a non-colliding bundled omnipowers skill is still available
echo ""
echo "Test 4: Testing non-colliding omnipowers skill remains available..."

mkdir -p "$OMNIPOWERS_SKILLS_DIR/omnipowers-only-test"
cat > "$OMNIPOWERS_SKILLS_DIR/omnipowers-only-test/SKILL.md" <<'EOF'
---
name: omnipowers-only-test
description: Omnipowers-only priority test skill
---
# Omnipowers Only Test Skill

PRIORITY_MARKER_OMNIPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"omnipowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_OMNIPOWERS_ONLY_VERSION" "Non-colliding omnipowers skill is still registered"
