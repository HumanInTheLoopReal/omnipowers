...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/omnipowers.js"
...
skill_count=$(find "$OMNIPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $OMNIPOWERS_SKILLS_DIR"
    exit 1
fi

# Test 3: Check using-omnipowers skill exists (critical for bootstrap)
echo "Test 3: Checking using-omnipowers skill (required for bootstrap)..."
if [ -f "$OMNIPOWERS_SKILLS_DIR/using-omnipowers/SKILL.md" ]; then
...
if node --check "$OMNIPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/omnipowers/' "$OMNIPOWERS_PLUGIN_FILE"; then
