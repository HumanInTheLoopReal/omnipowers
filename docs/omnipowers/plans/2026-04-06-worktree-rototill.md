...
**Goal:** Make omnipowers defer to native harness worktree systems when available, fall back to manual git worktrees when not, and fix three known finishing bugs.
...
**Spec:** `docs/omnipowers/specs/2026-04-06-worktree-rototill-design.md`
...
**If worktree path is under `.worktrees/` or `worktrees/`:** Omnipowers created this worktree — we own cleanup.
...
```markdown
- **omnipowers:using-git-worktrees** - Ensures isolated workspace (creates one or verifies existing)
```
